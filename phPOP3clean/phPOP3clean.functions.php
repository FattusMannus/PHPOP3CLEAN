...
function QuotedOrBinaryStringDecode($string) {
	if (preg_match(preg_expression('^=\\?([a-z0-9\\-]+)\\?([qb])\\?(.*)\\?=$', 'is'), trim($string), $matches)) {
		...
		switch (strtoupper($matches[2])) {
			case 'Q':
				//  Subject: =?iso-8859-1?Q?vi=E0gra!?=                   // vi�gra
				//  Subject: =?iso-8859-1?Q?Test_with_=E0_special_char?=  // vi�gra
				$string = QuotedEntityDecode($SubjectData);
				break;

			case 'B':
				//  Subject: =?iso-8859-1?B?4A==?=  �
				//  Subject: =?iso-8859-1?B?4OA=?=  ��
				//  Subject: =?iso-8859-1?B?4Q==?=  �
...
function ObfuscatedWordLists() {
	...
	if (count($Obfusnumeration) === 0) {
		$Obfusnumeration = array(
			'0' => PHPOP3CLEAN_OBFUSPACE.'*([0o������������]|zero)'.PHPOP3CLEAN_OBFUSPACE.'*',
...
function linkencode($url) {
	...
	if (empty($query) === false) {
		...
		// modified version by James Heinrich <info�silisoftware*com> 13-Dec-2006
/*		$query_array = array();
